...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("ingestion_run_id", UUID(as_uuid=False), sa.ForeignKey("card_ingestion_runs.id"), nullable=False),
        sa.Column("raw_json", JSONB, nullable=False),
        sa.Column("ingested_at", sa.DateTime, nullable=False),
        sa.Column("oracle_text_checksum", sa.String(64), nullable=False),
    )
    op.create_index("ix_raw_scryfall_cards_oracle_id", "raw_scryfall_cards", ["oracle_id"])
    op.create_index("ix_raw_scryfall_cards_run_oracle", "raw_scryfall_cards", ["ingestion_run_id", "oracle_id"])
    ...
    op.create_table(
        ...
        sa.Column("is_planeswalker", sa.Boolean, nullable=False, server_default="false"),
    ...
    op.create_table(
        "card_legalities",
        sa.Column("id", UUID(as_uuid=False), server_default=sa.text("gen_random_uuid()"), primary_key=True),
        sa.Column("oracle_id", UUID(as_uuid=False), sa.ForeignKey("cards.oracle_id"), nullable=False),
        sa.Column("format", sa.String(32), nullable=False),
        sa.Column("status", sa.String(16), nullable=False),
        sa.UniqueConstraint("oracle_id", "format", name="uq_card_legalities_oracle_format"),
    )
    ...
    op.create_table(
        ...
        sa.Column("loyalty", sa.String(8), nullable=True),
        sa.Column("colors", ARRAY(sa.String), nullable=False, server_default="{}"),
        sa.UniqueConstraint("oracle_id", "face_index", name="uq_card_faces_oracle_face"),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("oracle_id", "keyword", name="uq_card_keywords_oracle_keyword"),
    )
    ...
    op.create_index("ix_classification_jobs_oracle_id", "card_classification_jobs", ["oracle_id"])
    op.create_index("ix_classification_jobs_status", "card_classification_jobs", ["status"])

    op.create_table(
        "card_classifications",
        sa.Column("id", UUID(as_uuid=False), server_default=sa.text("gen_random_uuid()"), primary_key=True),
        sa.Column("oracle_id", UUID(as_uuid=False), sa.ForeignKey("cards.oracle_id"), nullable=False),
        sa.Column("job_id", UUID(as_uuid=False), sa.ForeignKey("card_classification_jobs.id"), nullable=False),
    ...
    op.create_index("ix_card_classifications_oracle_id", "card_classifications", ["oracle_id"])

    op.create_table(
        "card_functions",
        sa.Column("id", UUID(as_uuid=False), server_default=sa.text("gen_random_uuid()"), primary_key=True),
        sa.Column("oracle_id", UUID(as_uuid=False), sa.ForeignKey("cards.oracle_id"), nullable=False),
        sa.Column("classification_id", UUID(as_uuid=False), sa.ForeignKey("card_classifications.id"), nullable=False),
    ...
    op.create_index("ix_card_archetype_scores_oracle_id", "card_archetype_scores", ["oracle_id"])
    ...
    op.create_index("ix_card_power_scores_oracle_id", "card_power_scores", ["oracle_id"])
    ...
    op.create_index("ix_card_bracket_scores_oracle_id", "card_bracket_scores", ["oracle_id"])
    ...
    op.create_index("ix_card_synergy_hooks_oracle_id", "card_synergy_hooks", ["oracle_id"])
    ...
    op.create_table(
        ...
        sa.Column("typical_colors", ARRAY(sa.String), nullable=False, server_default="{}"),
        sa.Column("key_mechanics", ARRAY(sa.String), nullable=False, server_default="{}"),
    )
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=False), server_default=sa.text("gen_random_uuid()"), primary_key=True),
        sa.Column("name", sa.Text, nullable=False),
        sa.Column("commander_oracle_id", UUID(as_uuid=False), sa.ForeignKey("cards.oracle_id"), nullable=True),
        sa.Column("partner_oracle_id", UUID(as_uuid=False), sa.ForeignKey("cards.oracle_id"), nullable=True),
        sa.Column("colors", ARRAY(sa.String), nullable=False, server_default="{}"),
        sa.Column("created_at", sa.DateTime, nullable=False),
    ...
    op.create_table(
        "deck_cards",
        sa.Column("id", UUID(as_uuid=False), server_default=sa.text("gen_random_uuid()"), primary_key=True),
        sa.Column("decklist_id", UUID(as_uuid=False), sa.ForeignKey("decklists.id"), nullable=False),
        sa.Column("oracle_id", UUID(as_uuid=False), sa.ForeignKey("cards.oracle_id"), nullable=False),
...
def downgrade() -> None:
    for table in [
        "deck_evaluations", "deck_cards", "decklists",
        "bracket_context", "commander_rules_context", "archetypes",
        "card_synergy_hooks", "card_bracket_scores", "card_power_scores",
        "card_archetype_scores", "card_functions", "card_classifications",
        "card_classification_jobs", "card_keywords", "card_faces",
        "card_legalities", "cards", "raw_scryfall_cards", "card_ingestion_runs",
    ]:
        op.drop_table(table)
