...
def _results_description(variant: dict) -> str | None:
    ...
    effects = [p.get("feature", {}).get("name", "") for p in produces if p.get("feature")]
    return "; ".join(e for e in effects if e) or None


def run_spellbook_ingestion(session: Session) -> SpellbookIngestionResult:
    now = datetime.utcnow()
    result = SpellbookIngestionResult(combos_upserted=0, combo_cards_inserted=0, pages_fetched=0)
    ...
    with httpx.Client(headers={"User-Agent": "mtg-evaluator/1.0 (research)"}) as client:
        ...
        print("  Fetching all combos (2s/page, ~3-5 min for full dataset)...", flush=True)
        ...
        while next_url:
            ...
            for variant in variants:
                ...
                for card_data in card_entries:
                    ...
                    session.add(SpellbookComboCard(
            ...
            if result.pages_fetched % 5 == 0:
                ...
                print(f"    page {result.pages_fetched} — {result.combos_upserted} combos stored", flush=True)
