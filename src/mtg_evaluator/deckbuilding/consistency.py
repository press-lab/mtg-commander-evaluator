...
class ConsistencyReport:
    ...
    def warnings(self) -> list[str]:
        ...
        if self.lands_in_opener < 0.80:
            w.append(f"Low land count ({self.land_count}): only {self.lands_in_opener:.0%} chance of 2+ lands in opener. Add 2-3 more lands.")
        if self.ramp_by_turn3 < 0.70:
            w.append(f"Low ramp ({self.ramp_count} pieces): only {self.ramp_by_turn3:.0%} chance of ramp by turn 3. Add 2-3 ramp pieces.")
        if self.commander_on_curve < 0.60:
            w.append(f"Commander cast reliability low ({self.commander_on_curve:.0%}): CMC {self.commander_cmc:.0f} is hard to cast on curve with {self.land_count} lands.")
        if self.interaction_by_turn5 < 0.75:
            w.append(f"Low interaction ({self.interaction_count} pieces): only {self.interaction_by_turn5:.0%} chance of answer by turn 5.")
        return w
...
def compute_consistency(
    ...
    return ConsistencyReport(
        ...
        commander_on_curve=prob_commander_on_curve(commander_cmc, land_count, deck_size),
        interaction_by_turn5=prob_interaction_by_turn(interaction_count, deck_size),
        enabler_payoff_by_turn6=prob_enabler_and_payoff(enabler_count, payoff_count, deck_size),
