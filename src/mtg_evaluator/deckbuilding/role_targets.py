...
_BASE_TARGETS: dict[int, dict[str, int]] = {
    1: {"lands": 38, "ramp": 10, "draw": 8,  "removal": 5,  "board_wipe": 2, "protection": 2,  "finisher": 3},
    2: {"lands": 37, "ramp": 11, "draw": 9,  "removal": 6,  "board_wipe": 2, "protection": 2,  "finisher": 3},
    3: {"lands": 36, "ramp": 12, "draw": 10, "removal": 7,  "board_wipe": 2, "protection": 3,  "finisher": 4},
    4: {"lands": 32, "ramp": 14, "draw": 10, "removal": 6,  "board_wipe": 2, "protection": 3,  "finisher": 4},
    5: {"lands": 29, "ramp": 16, "draw": 11, "removal": 5,  "board_wipe": 1, "protection": 4,  "finisher": 3},
}
...
class RoleTargets:
    ...
    def to_dict(self) -> dict[str, int]:
        return {
            "lands": self.lands, "ramp": self.ramp, "draw": self.draw,
            "removal": self.removal, "board_wipe": self.board_wipe,
            "protection": self.protection, "finisher": self.finisher,
            **self.extra,
        }
...
def compute_role_targets(
    ...
    if profile:
        ...
        if profile.provides_protection:
            ...
            mods.append(f"protection {delta:+d} ({profile.card_name} has built-in protection)")

        if profile.provides_wincon:
            delta = -2
            targets["finisher"] = targets.get("finisher", 3) + delta
            mods.append(f"finisher {delta:+d} ({profile.card_name} is the win condition)")
        ...
        if profile.protection_need >= 4:
            delta = +2
            targets["protection"] = targets["protection"] + delta
            mods.append(f"protection {delta:+d} (high protection need, score={profile.protection_need})")
