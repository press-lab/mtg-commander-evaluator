...
from mtg_evaluator.deckbuilding.pool import _composite_score, _log_popularity, CardPool, PoolCard
...
class TestDeckRequest:
    ...
    def test_b1_b2_only_casual_tags(self):
        for bracket in (1, 2):
            ...
            for disallowed in ("R", "S", "P"):
                assert disallowed not in req.allowed_combo_tags, \
    ...
    def test_explicit_allowed_tags_not_overwritten(self):
        ...
        req = DeckRequest(
            commander_name="Test", bracket=3, want_combos=True,
...
class TestCardPoolProperties:
    ...
    def _make_pool_card(self, name, tier="CORE"):
        return PoolCard(
            name=name, oracle_id=f"{name}-oid", tier=tier, score=50.0,
            archetype_score=3.0, bracket_score=3.0, functions=[],
            is_game_changer=False, combo_ids=[], edhrec_decks=1000,
            type_line="Creature",
        )

    def test_all_cards_combines_tiers(self):
        req = DeckRequest(commander_name="X")
        pool = CardPool(request=req, commander_name="X", color_identity=["B"], archetype="sac")
        pool.core = [self._make_pool_card("A")]
        pool.support = [self._make_pool_card("B", "SUPPORT"), self._make_pool_card("C", "SUPPORT")]
    ...
    def test_empty_pool_total_zero(self):
        req = DeckRequest(commander_name="X")
        pool = CardPool(request=req, commander_name="X", color_identity=[], archetype=None)
        assert pool.total == 0
...
class TestFindCommandersIntegration:
    ...
    def test_color_filter_restricts_results(self, session):
        ...
        results = find_commanders(session, archetype="sacrifice", colors=["B"], limit=20)
        for c in results:
            ci = set(c.color_identity) - {"C"}
            assert ci.issubset({"B"}), f"{c.name} has off-color identity: {c.color_identity}"
    ...
    def test_non_creature_legendaries_excluded(self, session):
        ...
        for c in results:
            assert "Creature" in c.type_line, \
...
class TestBrowseCardsIntegration:
    ...
    def test_browse_sacrifice_ramp(self, session):
        ...
        results = browse_cards(session, archetype="sacrifice", bracket=3, role="ramp", limit=20)
        assert len(results) > 0

    def test_tutors_pass_any_role_filter(self, session):
        from mtg_evaluator.deckbuilding.browse import browse_cards
        # When filtering for ramp, tutors should still appear
        results = browse_cards(session, archetype="sacrifice", bracket=3, role="ramp", limit=50)
        names = [c.name for c in results]
        ...
        assert len(tutors_in_result) > 0, "No tutors in ramp results — tutor bypass broken"
    ...
    def test_color_filter_excludes_off_color(self, session):
        from mtg_evaluator.deckbuilding.browse import browse_cards
        results = browse_cards(session, archetype=None, bracket=3, colors=["W"], limit=50)
        for c in results:
            if c.oracle_id:
...
class TestBuildCardPoolIntegration:
    ...
    def test_commander_not_in_pool(self, session):
        ...
        req = DeckRequest(
            commander_name='Henzie "Toolbox" Torre',
            bracket=4, archetype="midrange", want_combos=False,
        )
        pool = build_card_pool(session, req)
    ...
    def test_gc_tutors_present_at_b4(self, session):
        ...
        req = DeckRequest(
            commander_name='Henzie "Toolbox" Torre',
            bracket=4, archetype="midrange", want_combos=False, tutor_density="light",
        )
        pool = build_card_pool(session, req)
        all_names = [c.name for c in pool.all_cards]
        assert "Demonic Tutor" in all_names or "Vampiric Tutor" in all_names, \
    ...
    def test_gc_tutors_absent_at_b2(self, session):
        ...
        req = DeckRequest(
            ...
            bracket=2, archetype="control", want_combos=False, tutor_density="light",
        )
        try:
            pool = build_card_pool(session, req)
        except ValueError:
        ...
        for card in pool.all_cards:
            assert card.name not in GAME_CHANGERS, \
    ...
    def test_pool_respects_size(self, session):
        ...
        req = DeckRequest(
            commander_name='Henzie "Toolbox" Torre',
            bracket=4, archetype="midrange", want_combos=False, pool_size=50,
        )
        pool = build_card_pool(session, req)
