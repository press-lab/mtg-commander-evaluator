...
with get_session() as s:
    for name in CARDS:
        ...
        brackets = {
            b.bracket_level: b.score
            for b in s.scalars(
                select(CardBracketScore).where(
                    CardBracketScore.classification_id == cls.id
                )
            ).all()
        }
        gp = cls.raw_output.get("general_commander_power")
        top_arch = sorted(
            [(k, v) for k, v in cls.raw_output.get("archetype_fit", {}).items() if v],
            key=lambda x: x[1],
            reverse=True,
        )[:3]
        ...
        print(
            f"  power     : {gp}/5  |  casual={b.get('casual')} b2={b.get('bracket_2')} b3={b.get('bracket_3')} b4={b.get('bracket_4')} cedh={b.get('cedh')}"
        )
