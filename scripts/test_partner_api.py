...
print(
    f"Partner label: {info['partner_info']['label'] if info['partner_info'] else 'none'}"
)

# Test Shadowheart (Choose a Background)
sh = httpx.get(
    "http://127.0.0.1:8000/api/search/commanders?q=shadowheart", timeout=10
).json()
if sh:
    sh_info = httpx.get(
        f"http://127.0.0.1:8000/api/commander-info/{sh[0]['oracle_id']}", timeout=10
    ).json()
    sh_partners = httpx.get(
        f"http://127.0.0.1:8000/api/partners/{sh[0]['oracle_id']}", timeout=10
    ).json()
    print(f"\nShadowheart partner type: {sh_info['partner_info']['type']}")
    print(
        f"Background options (first 5): {[p['name'] for p in sh_partners['valid_partners'][:5]]}"
    )
